...
class AnalyticsService {
  ...
  private getUsersFromStorage(): any[] {
    try {
      // Simuler la récupération des utilisateurs depuis les contextes
      const mockUsers = [
        { id: '1', role: 'admin', status: 'active', joinDate: '2024-01-15' },
        { id: '2', role: 'teacher', status: 'active', joinDate: '2024-02-20' },
        { id: '3', role: 'teacher', status: 'active', joinDate: '2024-03-10' },
        { id: '4', role: 'student', status: 'active', joinDate: '2024-11-01' },
        { id: '5', role: 'student', status: 'active', joinDate: '2024-11-15' },
        { id: '6', role: 'student', status: 'active', joinDate: '2024-12-01' },
        { id: '7', role: 'student', status: 'active', joinDate: '2024-12-10' },
        { id: '8', role: 'student', status: 'active', joinDate: '2024-12-15' }
      ];
      
      // Générer des utilisateurs supplémentaires pour avoir des statistiques réalistes
      const additionalUsers = Array.from({ length: 1200 }, (_, i) => ({
        id: `user_${i + 9}`,
        role: Math.random() > 0.8 ? 'teacher' : 'student',
        status: Math.random() > 0.1 ? 'active' : 'inactive',
        joinDate: new Date(2020 + Math.floor(Math.random() * 5), 
  ...
  private getProjectsFromStorage(): any[] {
    try {
      // Récupérer les projets depuis le contexte existant
      const existingProjects = JSON.parse(localStorage.getItem('projects') || '[]');
      
      // Ajouter des projets simulés pour avoir des statistiques réalistes
      const additionalProjects = Array.from({ length: 80 }, (_, i) => ({
        id: `proj_${i + 10}`,
        category: ['Développement Web', 'IA & Machine Learning', 'Cybersécurité', 'Data Science'][Math.floor(Math.random() * 4)],
      ...
      return [...existingProjects, ...additionalProjects];
    } catch {
      return [];
    }
  }
  ...
  public getAnalyticsData(filters?: AnalyticsFilters): AnalyticsData | null {
    ...
    if (filters) {
      return this.applyFilters(this.analyticsData!, filters);
    }
    
    return this.analyticsData;
  }

  private applyFilters(data: AnalyticsData, filters: AnalyticsFilters): AnalyticsData {
